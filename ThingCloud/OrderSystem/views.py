...
def generateOrder(request):
    ...
    if not _addr:
        order = Order(user=_user, notes="", fee=0, typeid=typeid, itemList=itemlist, state=0, create_time=createtime)
        order.save()
        return Jsonify({"status":True, "error":"", "error_message":"", "order":order, "address":""})
    else:
        _addr_object = _addr[0]
        _addr = model_to_dict(_addr_object)
        order = Order(notes="", fee=6, typeid=typeid, itemList=itemlist, state=0, create_time=createtime, addr=_addr_object)
        order.save()
...
def complain(request):
    ...
    if not _order:
        ...
        _order = _order[0]
        comp = Complaint(oid=oid, user_id=_user["uid"], notes=notes, state=0)
        comp.save()
        _order.state = 9
        _order.save()
