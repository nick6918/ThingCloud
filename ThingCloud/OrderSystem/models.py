...
def addInfo(order, state):
    ...
    if state < 2:
        ...
        if typeid == 0:
            ...
            html_text += "  <p>正在等待快递员出单, 预计30分钟内即将您的订单送至<span class=info>"+order.address.name+"。</span></p>\n"
    elif state==3:
        html_text = html_text + addInfo(order, 2)
        if typeid == 0:
            html_text += "  <p>快递员<span class=info>"+order.courier.worker.name+"</span>已出单, 预计20分钟内前往<span class=info>"+order.address.name+"</span>取件。</p>\n"
        else:
            html_text += "  <p>快递员<span class=info>"+order.courier.worker.name+"</span>已出单, 预计30分钟内即将您的订单送至<span class=info>"+order.address.name+"</span>。</p>\n"
    elif state==4:
        html_text = html_text + addInfo(order, 3)
        html_text += "  <p>快递员已取到货品, 预计1小时内送货入库。货品入库后, 您可在订单页面或者邻仓主页查看所有您存入的商品。</p>\n"
    elif state==5:
        if typeid == 0:
            html_text = html_text + addInfo(order, 4)
            html_text += "  <p>快递员已上传存货单, <a href="+PICURL+str(order.oid)+ ".png >点此查看</a>。</p>\n"
            html_text += "  <p>存货成功，快递员已上传全部商品， 请前往主页确认。如有疑问, 请联系客服。</p>\n"
        else:
            html_text = html_text + addInfo(order, 3)
            html_text += "  <p>快递员已上传存货单, <a href="+PICURL+str(order.oid)+ ".png >点此查看</a>。</p>\n"
            html_text += "  <p>取件成功， 快递员已将商品送至指定地址， 请您确认。如有疑问, 请联系客服。</p>\n"
    elif state == 7 or state == 8:
        html_text = html_text + addInfo(order, 3)
        html_text += "  <p>您的订单已取消。如有疑问, 请联系客服。</p>\n"
    elif state == 9:
        html_text = html_text + addInfo(order, 5)
        html_text += "  <p>系统已收到您的申诉请求, 我们将于1天内与您取得联系。</p>\n"
    elif state == 6:
        html_text = html_text + addInfo(order, 5)
        html_text += "  <p>您已确认订单无误， 订单完成。谢谢您的使用。</p>\n"
    else:
        html_text = html_text + addInfo(order, 3)
        html_text += "  <p>您的订单已取消。如有疑问, 请联系客服。</p>\n"
    return html_text
